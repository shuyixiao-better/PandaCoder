...
```javascript
{
  ...
  "userDepartment": "研发部"                   // 部门
}
```

## 🔍 使用场景

1. **团队周报统计**: 通过用户编码统计每个成员的周报提交情况
2. **跨设备识别**: 通过用户编码识别同一用户在不同设备上的周报
3. **数据审计**: 通过设备ID和用户信息进行数据追溯
4. **部门统计**: 通过部门字段进行团队级别的统计分析

## 🚀 后续优化建议

1. **批量导入用户信息**: 支持从 CSV 或 Excel 导入用户信息
2. **用户信息同步**: 支持从企业 LDAP 或 AD 同步用户信息
3. **多设备管理**: 在配置界面显示用户的所有设备
4. **数据统计面板**: 在插件中添加用户周报统计面板
5. **导出功能**: 支持导出用户的所有周报为 PDF 或 Word
