 ...
 */
@Service
public final class ErrorPatternLearningService {

    private static final Logger LOG = Logger.getInstance(ErrorPatternLearningService.class);
    ...
    public ErrorPatternLearningService(@NotNull Project project) {
        ...
        LOG.info("Error Pattern Learning Service initialized for project: " + project.getName());
    }
    ...
    private void startTrainingTask() {
        trainingScheduler.scheduleWithFixedDelay(() -> {
            try {
                ...
                LOG.warn("Error during model training", e);
            }
    ...
    public void shutdown() {
        LOG.info("Shutting down Error Pattern Learning Service for project: " + project.getName());
        ...
        LOG.info("Error Pattern Learning Service shutdown completed for project: " + project.getName());
    }
    ...
    private static class LearnedPattern {
        ...
        public boolean isExpired() {
            // 模式如果超过30天未出现，则认为过期
            return System.currentTimeMillis() - lastSeenTimestamp > MAX_PATTERN_AGE_DAYS * 24 * 60 * 60 * 1000L;
        }
    }
}