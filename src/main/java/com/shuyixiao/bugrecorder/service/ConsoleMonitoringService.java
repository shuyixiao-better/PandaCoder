...
import com.shuyixiao.bugrecorder.listener.ConsoleOutputListener;
 ...
 */
@Service
public final class ConsoleMonitoringService {

    private static final Logger LOG = Logger.getInstance(ConsoleMonitoringService.class);

    private final Project project;
    private final Map<ProcessHandler, ConsoleOutputListener> activeListeners = new ConcurrentHashMap<>();
    private boolean monitoringEnabled = true;

    public ConsoleMonitoringService(@NotNull Project project) {
    ...
    public void enableMonitoring() {
        ...
        LOG.info("Console monitoring enabled for project: " + project.getName());
    }
    ...
    public void disableMonitoring() {
        ...
        // 清理所有活动的监听器
        clearAllListeners();
        LOG.info("Console monitoring disabled for project: " + project.getName());
    }
    ...
    public void attachListener(@NotNull ProcessHandler processHandler) {
        ...
        try {
            ...
            if (activeListeners.containsKey(processHandler)) {
                LOG.debug("Listener already exists for process: " + processHandler);
                return;
            }

            // 创建新的监听器
            ConsoleOutputListener listener = new ConsoleOutputListener(project);
            processHandler.addProcessListener(listener);
            activeListeners.put(processHandler, listener);

            LOG.debug("Attached console listener to process: " + processHandler);
            ...
            LOG.error("Failed to attach console listener", e);
        }
    }
    ...
    public void removeListener(@NotNull ProcessHandler processHandler) {
        ConsoleOutputListener listener = activeListeners.remove(processHandler);
        if (listener != null) {
            try {
                processHandler.removeProcessListener(listener);
                LOG.debug("Removed console listener from process: " + processHandler);
            } catch (Exception e) {
                LOG.warn("Failed to remove console listener", e);
            }
        }
    }
    ...
    public void clearAllListeners() {
        for (Map.Entry<ProcessHandler, ConsoleOutputListener> entry : activeListeners.entrySet()) {
            try {
        ...
        LOG.debug("Cleared all console listeners");
    }
