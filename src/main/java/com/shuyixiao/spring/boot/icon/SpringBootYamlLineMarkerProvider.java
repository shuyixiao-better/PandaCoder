...
/**
 * Spring Boot YAML配置文件行标记提供器
 * 智能识别配置内容并显示相应的技术栈图标
 */
public class SpringBootYamlLineMarkerProvider implements LineMarkerProvider {

    private static final Logger LOG = Logger.getInstance(SpringBootYamlLineMarkerProvider.class);
    ...
    private static class TechStackConfig {
        ...
        public TechStackConfig(String iconPath, String displayName, List<String> configKeywords, List<String> urlKeywords) {
            ...
            for (String keyword : configKeywords) {
                ...
                // 嵌套配置：spring.keyword 或 spring: keyword:
                configPatterns.add(Pattern.compile("(^|\\s+)" + keyword + "\\s*:", Pattern.CASE_INSENSITIVE));
            }
    ...
    private static Icon loadIcon(String path) {
        try {
            Icon icon = IconLoader.getIcon(path, SpringBootYamlLineMarkerProvider.class);
